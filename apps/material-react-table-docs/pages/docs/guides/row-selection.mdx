...
By default, a row can only be selected by either clicking the checkbox or radio button in the `__check__` column. If you want to be able to select a row by clicking anywhere on the row, you can add your own `onClick` function to a table body row like this:
