...
By default, column hiding is only enabled on data columns. Display columns, such as `mrt-row-numbers`, `__check__`, etc., do not have column hiding enabled, and their toggle will be disabled. You can turn that back on by setting the `enableHiding` option to `true` in the `displayColumnsOptions` table option.
